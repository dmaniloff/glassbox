...
class SVDTritonAttentionImpl(TritonAttentionImpl):
    ...
    def set_config(cls, config: GlassboxConfig) -> None:
        ...
        # Drop the cached matvec strategy so a new config's choice takes effect.
        if hasattr(cls, "_resolved_strategy"):
            del cls._resolved_strategy
        ...
        for head_idx in sorted(heads):
            ...
            for sig_name in due_signals:
                ...
                result = diag.reduce(
                    ...
                    Kh,
                    L,
                    prior_state=state.accum.get(sig_name),
