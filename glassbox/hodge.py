...
def estimate_curl_materialized(M, target_cv=0.05, seed=42):
    ...
    C = rms / (math.sqrt(2) * M_fro + 1e-12)
    return C.item()


def estimate_curl_matrix_free(Q, K, lse, d_k_inv_sqrt, scale, M_fro_norm, target_cv=0.05, seed=42):
    ...
    rms = circs.square().mean().sqrt()
    C = rms / (math.sqrt(2) * M_fro_norm + 1e-12)
    return C.item()


def compute_G_materialized(M):
    """Asymmetry coefficient: G = ||M_asym||_F / ||M||_F."""
    M_fro = torch.linalg.norm(M, "fro")
    M_asym = (M - M.T) / 2.0
    M_asym_fro = torch.linalg.norm(M_asym, "fro")
    G = (M_asym_fro / (M_fro + 1e-12)).item()
    return G, M_fro.item()
...
def compute_G_matrix_free(Q, K, d_k_inv_sqrt, scale, block_size=256):
    ...
    G = (torch.sqrt(asym_sq) / (torch.sqrt(norm_sq) + 1e-12)).item()
    return G, M_fro


def compute_routing_features_materialized(M, rank, svd_method="randomized", target_cv=0.05, seed=42):
    """Below-threshold all-in-one: materialize everything."""
    n = M.shape[0]
    sigma = torch.linalg.svdvals(M)
    sigma2 = (sigma[:rank] ** 2).sum().item() / ((sigma ** 2).sum().item() + 1e-12)
    phi_hat = 1.0 - sigma2
    ...
    sigma2_asym = (sigma_asym[:rank] ** 2).sum().item() / ((sigma_asym ** 2).sum().item() + 1e-12) if (sigma_asym ** 2).sum().item() > 1e-12 else 0.0

    # Commutator norm
    comm = M @ M.T - M.T @ M
    commutator_norm = torch.linalg.norm(comm, "fro").item() / (M_fro ** 2 + 1e-12)
...
def compute_routing_features_matrix_free(Q, K, d_k_inv_sqrt, scale, lse, rank,
                                          svd_method="randomized", block_size=256,
                                          target_cv=0.05, seed=42):
    ...
    # For phi_hat we need ||M||_F as a proxy for total spectral energy
    M_fro_norm = compute_M_fro_norm_blocked(Q, K, d_k_inv_sqrt, scale, block_size)
    M_fro_val = M_fro_norm.item()
    total_energy = M_fro_val ** 2
    top_energy = (S ** 2).sum().item()
    sigma2 = top_energy / (total_energy + 1e-12)
    phi_hat = 1.0 - sigma2

    G, _ = compute_G_matrix_free(Q, K, d_k_inv_sqrt, scale, block_size)

    C = estimate_curl_matrix_free(Q, K, lse, d_k_inv_sqrt, scale, M_fro_val, target_cv, seed)
