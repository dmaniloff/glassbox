...
Triton is an **optional** dependency (``pip install glassbox[triton]``) and is
only useful on CUDA.  This module imports cleanly without it; callers must gate
on ``HAS_TRITON`` (and a CUDA device) before calling ``fused_attn_multi_matvec``.
The matrix-free diagnostic paths fall back to the blocked PyTorch matvecs in
``glassbox/svd.py`` when Triton is absent or a kernel launch fails.
"""
...
if HAS_TRITON:
    ...
    def fused_attn_multi_matvec(
        ...
        O(L_q × L_k).  Single kernel launch vs (L/block_size) blocked passes.
...
else:  # pragma: no cover - exercised only on non-Triton hosts

    def fused_attn_multi_matvec(*args, **kwargs):
        """Stub raised when Triton is unavailable; callers should gate on HAS_TRITON."""
        raise RuntimeError(
            "fused_attn_multi_matvec requires Triton (pip install 'glassbox[triton]') "
            "and a CUDA device. Gate on glassbox.triton_kernels.HAS_TRITON before calling."
        )
