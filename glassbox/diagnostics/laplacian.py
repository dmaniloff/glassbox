"""Laplacian eigenvalue diagnostic (LapEigvals, EMNLP 2025)."""
...
class LaplacianDiagnostic:
    ...
    def reduce(self, Qh: torch.Tensor, Kh: torch.Tensor, L: int, **ctx: Any) -> dict:
        ...
        if L <= self.threshold:
            ...
            tier = "materialized"
            features = compute_laplacian_eigvals_materialized(A, top_k=self.top_k)
        else:
            tier = "matrix_free"
    ...
    def witness(self, Qh: torch.Tensor, Kh: torch.Tensor, L: int, **ctx: Any) -> torch.Tensor:
        raise NotImplementedError("LaplacianDiagnostic witness not yet implemented")
