...
on `P` because the degree normalization is an *asymmetric* scaling (`D_Q != D_K`) that
does not preserve antisymmetry and inflates the rank of the antisymmetric part
(ShadeFormal `NormalizationInvariance`), muddying the gradient = degree-imbalance reading
that is clean on `P`.  This matches the streaming-asym-operators paper, which decomposes
`A = (P - P^T)/2`.  Conductance/Cheeger keeps `M` (its normalization is theorem-required).
...
Streaming (issue #39).  G is a *ratio* of Frobenius norms, so it is not additive across
windows.  Its components are: ``||P_asym||_F^2`` and ``||P||_F^2`` are both sums-of-squares
over matrix entries, hence additive over a *disjoint* partition of windows.  ``reduce()``
reports the global statistic ``G = sqrt(S_asym) / sqrt(S_den)`` from running sufficient
statistics folded out of ``prior_state``, and ``accumulate()`` persists them.  This is
unbiased only under disjoint (tumbling) windowing; under sliding (overlapping) windows the
overlap is double-counted.  The accumulated object is the exact G of the block-diagonal
operator over the processed stream — cross-window asymmetry is not captured.  The per-token
asymmetry profile is emitted per-window as the witness.

Performance: a window's scalar partials and per-row witness share ONE multi-RHS Hutchinson
pass, memoized so the backend's reduce()-then-witness() calls do not recompute it.  Streaming
windows offset the probe seed so per-window estimation errors decorrelate across the stream.
"""
